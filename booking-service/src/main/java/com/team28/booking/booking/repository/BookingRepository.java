...
public interface BookingRepository extends JpaRepository<Booking, Long> {
        ...
        default void updateProviderStatusToAvailable(Long providerId) {
                throw new UnsupportedOperationException("Provider status is owned by provider-service");
        }

        default void createInvoiceForBooking(Long bookingId, Long userId, BigDecimal amount) {
                throw new UnsupportedOperationException("Invoice creation is owned by invoice-service saga");
        }

        default Object[] getDashboardAggregates(LocalDateTime startDate, LocalDateTime endDate) {
                return new Object[]{0L, BigDecimal.ZERO, BigDecimal.ZERO};
        }

        default boolean existsUserById(Long userId) {
                throw new UnsupportedOperationException("User existence is owned by user-service");
        }

        default List<Object[]> findProvidersByIds(List<Long> providerIds) {
                throw new UnsupportedOperationException("Provider details are owned by provider-service");
        }
